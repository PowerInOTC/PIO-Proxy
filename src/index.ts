...
async function init(): Promise<void> {
  try {
    ...
    const assetPricesUpdater = WorkerController.getInstance(
      'priceWorker',
      './dist/workers/assetPricesUpdater.js',
    );
    if (
      !(await assetPricesUpdater.sendMessageToWorker({ type: 'setassets' }))
        .payload
    ) {
    ...
    app.get(
      '/api/v1/get_pair_price',
      limiter(config.windowTime, config.maxRequests),
    );
    ...
    server.listen(config.serverPort, () => {
      ...
      Logger.info(
        'app',
        `Server running on ${protocol}://${config.serverAddress}:${config.serverPort}`,
      );
    });
  } catch (error) {
    if (error instanceof Error) {
