...
export function loadApiKeys(filePath: string): string[] {
    try {
        ...
        if (error instanceof Error) {
            handleFunctionError('app', 'loadApiKeys', `Caught an error: ${error.message}`);
        }
        return [];
    }
}

export function checkAuthorization(req: express.Request, res: express.Response, next: express.NextFunction): void {
