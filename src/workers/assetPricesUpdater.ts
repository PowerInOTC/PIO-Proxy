...
import { formatAsset, getPairPrice, updateAssetPrices } from '../utils/priceUtils';
...
parentPort?.on('message', async (message: WorkerMessage) => {
    if (message.type === 'setassets') {
        ...
        if (!assetA || !checkAsset(assetA.prefix, assetA.assetName)) {
            parentPort?.postMessage({ uuid: message.uuid, type: "resultError", payload: `${message.payload.assetA} doesn't exists` });
            return;
        }

        if (!assetB || !checkAsset(assetB.prefix, assetB.assetName)) {
            parentPort?.postMessage({ uuid: message.uuid, type: "resultError", payload: `${message.payload.assetB} doesn't exists` });
            return;
        }
