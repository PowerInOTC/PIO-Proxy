...
class Alpaca {
  public static async getLatestPrices(
    ...
    try {
      ...
      const response: AxiosResponse<AlpacaResponse> = await axios.get(apiUrl, {
        headers: headers,
        timeout: 2000,
      });
      ...
      Object.entries(response.data.quotes).forEach(
        ([symbol, stock]: [string, AlpacaPrice]) => {
          if (stock.bp && stock.ap) {
            let timestamp = new Date(stock.t).getTime();
            if (timestamp < 1000000000000) {
              timestamp = Math.floor(timestamp * 1000);
            }
            prices[types[symbol.toUpperCase()] + '.' + symbol.toUpperCase()] = {
              symbol: symbol.toUpperCase(),
              type: types[symbol.toUpperCase()],
              bidPrice: stock.bp.toString(),
              askPrice: stock.ap.toString(),
              provider: 'alpaca',
              timestamp: timestamp,
            };
          }
